...
class BudgetRemoteDataSourceImpl implements BudgetRemoteDataSource {
  ...
  Future<Either<String, BudgetModel>> createBudget(BudgetParams budget) async {
    try {
      ...
      if (storedUserId == null || storedUserId.isEmpty) {
        ...
        return const Left('User ID is required to create a budget. Please log in again.');
      }
      print("Creating budget for user ID: $storedUserId");

      final options = await _getOptions();
      final response = await dio.post(ApiUrl.budgetURL,
          data: {"user": storedUserId, ...budget.toJson()}, options: options);
  ...
  Future<Either<String, void>> updateBudget(
    ...
    try {
      final options = await _getOptions();
      await dio.put(ApiUrl.dynamicBudgetURL(id),
          data: budget.toJson(), options: options);
  ...
  Future<Either<String, List<BudgetModel>>> getAllBudgets() async {
    try {
      ...
      final budgets =
          (response.data["budgets"] as List).map((e) => BudgetModel.fromJson(e)).toList();
  ...
  Future<Either<String, BudgetModel>> getBudgetById(String id) async {
    try {
      final options = await _getOptions();
      final response =
          await dio.get(ApiUrl.dynamicBudgetURL(id), options: options);
  ...
  Future<Either<String, Map<String, dynamic>>> getBudgetWithExpenses(
    ...
    try {
      final options = await _getOptions();
      final response = await dio.get(ApiUrl.dynamicBudgetWithExpensesURL(id),
          options: options);
  ...
  Future<Either<String, ExpenseModel>> getExpenseById(String id) async {
    try {
      final options = await _getOptions();
      final response =
          await dio.get(ApiUrl.dynamicExpenseURL(id), options: options);
  ...
  @override
  Future<Either<String, void>> createExpense(Map<String, dynamic> data) async {
    try {
      final options = await _getOptions();
      await dio.post(ApiUrl.expenseURL, data: data, options: options);
      return const Right(null);
    } on DioException catch (e) {
  ...
  Future<Either<String, void>> updateExpense(
    ...
    try {
      final options = await _getOptions();
      await dio.put(ApiUrl.dynamicExpenseURL(id), data: data, options: options);
      return const Right(null);
    } on DioException catch (e) {
